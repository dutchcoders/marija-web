export const OPEN_PANE = 'OPEN_PANE';
export const CLOSE_PANE = 'CLOSE_PANE';
export const SET_PANE_CONFIG = 'SET_PANE_CONFIG';
export const MOVE_PANE_TO_TOP = 'MOVE_PANE_TO_TOP';
export const OPEN_LIGHTBOX = 'OPEN_LIGHTBOX';
export const CLOSE_LIGHTBOX = 'CLOSE_LIGHTBOX';
export const RECEIVE_WORKSPACE_DESCRIPTIONS = 'RECEIVE_WORKSPACE_DESCRIPTIONS';
export const REQUEST_WORKSPACE = 'REQUEST_WORKSPACE';
export const RECEIVE_WORKSPACE = 'RECEIVE_WORKSPACE';
export const SAVE_WORKSPACE_IN_LOCAL_STORAGE = 'SAVE_WORKSPACE_IN_LOCAL_STORAGE';
export const SET_EXPERIMENTAL_FEATURES = 'SET_EXPERIMENTAL_FEATURES';
export const SET_REDUCER_ERROR = 'SET_REDUCER_ERROR';
export const SET_LANG = 'SET_LANG';
export const CURRENT_WORKSPACE_VERSION = 7;
export const EDIT_WORKSPACE_TITLE = 'EDIT_WORKSPACE_TITLE';

export const colors = [
	'#f44336',
	'#e91e63',
	'#ab47bc',
	'#7e57c2',
	'#5c6bc0',
	'#2196f3',
	'#03a9f4',
	'#00bcd4',
	'#009688',
	'#4caf50',
	'#8bc34a',
	'#cddc39',
	'#ffeb3b',
	'#ffc107',
	'#ff9800',
	'#ff5722',
	'#795548',
	'#9e9e9e',
	'#607d8b',
	'#000000',
];

export const queryColors = [
	'#ab47bc',
	'#5c6bc0',
	'#7e57c2',
	'#e91e63',
	'#ff9800',
	'#ff5722',
];

export const connectorColors = [
	'#2196f3',
	'#00bcd4',
	'#009688',
	'#4caf50',
	'#03a9f4',
	'#8bc34a',
	'#cddc39',
];